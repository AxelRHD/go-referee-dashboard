...
func DashboardPage(seasons []string, defaultSeason string) g.Node {
	...
	sidebar := h.Div(h.Class("col-md-2"),
		h.Div(g.Attr("style", "position:sticky;top:4rem"),
			...
			h.Div(h.Class("btn-group btn-group-sm w-100 mb-3"), g.Attr("role", "group"),
				h.Button(h.Class("btn"), g.Attr("type", "button"),
					g.Attr(":class", "chartLib === 'plotly' ? 'btn-secondary' : 'btn-outline-secondary'"),
					g.Attr("@click", "chartLib = 'plotly'"),
					g.Text("Plotly"),
				),
				h.Button(h.Class("btn"), g.Attr("type", "button"),
					g.Attr(":class", "chartLib === 'echarts' ? 'btn-secondary' : 'btn-outline-secondary'"),
					g.Attr("@click", "chartLib = 'echarts'"),
					g.Text("ECharts"),
				),
			),
			// Year view filters
	...
	yearCharts := h.Div(h.Class("col-md-10"), g.Attr("x-show", "view === 'year'"),
		...
		h.Div(g.Attr("x-show", "filtered.some(g => g.venue_lat && g.venue_lon && (g.venue_lat !== 0 || g.venue_lon !== 0))"),
	...
	overviewCharts := h.Div(h.Class("col-md-10"), g.Attr("x-show", "view === 'overview'"),
		...
		h.Div(g.Attr("x-show", "overviewFiltered.some(g => g.venue_lat && g.venue_lon && (g.venue_lat !== 0 || g.venue_lon !== 0))"),
